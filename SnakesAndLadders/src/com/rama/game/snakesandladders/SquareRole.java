...
/**
 * This class will tell the actual position of 
 * the player when the game is on.
 * @author rginde
 *
 */
public abstract class SquareRole {
	...
	public Square moveAndLand(int moves) {
		...
		if (presentPosition + moves > lastPosition) {
			...
			System.out.println(" move from " + (square.getPosition() + 1)
					+ " to "
					+ (square.findSquare(moves).getPosition() + 1));
			return square.findSquare(moves).findPosition();
		}
	}
